...
To protect the gift further, Parker controls what her own journal shows. She can toggle which categories (state, special, standard) appear in her Pantry, so the collection stays a celebration of what *she* chooses to celebrate, on her terms. The Encyclopedia is intentionally exempt from that toggle: as the reference half, it always shows every Trader Joe's bag, so what Parker hides from her own journal is never hidden from the wider catalog.
